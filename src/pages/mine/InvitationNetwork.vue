<template>
  <div class="invitation-network df fdc">
    ...
    <!-- 邀请分类 start -->
    <div class="pr">
      <ul class="df pr invite-wrap" ref="inviteWrap">
                <li
    ...
    >
      ...
      <div v-if="selectId===0||selectId===2">
        <div class="agent-level df jsb ac" v-if="selectId===0">
          <div>
            <span>一级代理</span>
            <svg
              class="icon"
      ...
      <div v-if="selectId===1">
          <ul class="container">
              <li v-for="(item,index) in inviteTeamList" :key="index" class="df jsb ac">
                  <div class="df ac">
                      <div class="img-wrap">
                          <img src="./../../../static/img/agent-img.jpg" alt="">
                      </div>
                      <div>
...
export default {
  data() {
    return {
      inviteStatusList: ["我邀请的", "邀请团队", "邀请我的"],
      scrollWidth: "100px",
      scrollLeft: 0,
      selectId: 0,
      inviteList: ["ATREUS"],
  ...
  mounted(){
      this.scrollWidth=`${this.$refs.inviteWrap.firstElementChild.clientWidth}px`
  },
  methods:{
      onItem(index,e){
          this.selectId=index;
